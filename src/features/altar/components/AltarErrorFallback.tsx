...
interface AltarErrorFallbackProps {
  error?: Error
  resetError?: () => void
}

export function AltarErrorFallback({ error, resetError }: AltarErrorFallbackProps) {
  return (
    <motion.div
      variants={fadeInUp}
      initial="hidden"
    ...
    >
      <div className="text-center max-w-md">
        ...
        <h2 className="font-cinzel font-bold text-24 text-text-primary mb-3">
          The Altar is Unavailable
        </h2>
        <p className="text-14 text-text-secondary mb-6">
          {error?.message || 'An unexpected error occurred while loading the altar. Please try again.'}
        </p>
