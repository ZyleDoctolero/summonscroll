...
describe('Game Logic Tests (QA-01)', () => {
  ...
  test('QA-02: minus task decreases HP by correct formula', async () => {
    // Formula check
    (supabase.rpc as any).mockResolvedValueOnce({ data: { hp_delta: -10, new_hp: 40 }, error: null });
  ...
  test('QA-04: pull with 0 crystals throws before roll', async () => {
    (supabase.rpc as any).mockResolvedValueOnce({ data: null, error: { message: 'Insufficient crystals' } });
  ...
  test('QA-07: task with realm_id produces non-empty growthTicks', async () => {
    (supabase.rpc as any).mockResolvedValueOnce({ data: { growth_ticks: [{ monster_id: '1', tick: 10 }] }, error: null });
  ...
  test('QA-09: ascend with insufficient gold throws error', async () => {
    (supabase.rpc as any).mockResolvedValueOnce({ data: null, error: { message: 'Insufficient gold' } });
  ...
  test('QA-11: empty compendium renders EmptyState, not error', async () => {
    ...
    const result = await supabase.from('user_monsters').select('*').eq('user_id', 'u1').order('level');
  ...
  test('QA-13: joining a guild twice returns error', async () => {
    (supabase.rpc as any).mockResolvedValueOnce({ data: null, error: { message: 'Already in guild' } });
  ...
  test('QA-15: streak freeze prevents HP loss on missed day', async () => {
    (supabase.rpc as any).mockResolvedValueOnce({ data: { hp_loss: 0, freeze_used: true }, error: null });
    const result = await supabase.rpc('run_daily_cron');
