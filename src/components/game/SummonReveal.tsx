...
const CEREMONY_CONFIG = {
  ...
  Legendary: { duration: dur.epic,     particles: true,  fullscreen: true,  color: '--rarity-legendary', goldRain: true },
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {current.isNew && (
            <p className="text-sm mt-2 font-medium flex items-center gap-1 justify-center" style={{ color: "var(--success)" }}>
          ...
          {sorted.map((res, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs font-bold truncate" style={{ color: "var(--ink-primary)" }}>{res.monster.name}</p>
                <span
                  className="ss-chip mt-1.5"
                  style={{ background: `${RARITY_COLOR[r]}20`, color: RARITY_COLOR[r], fontSize: '9px' }}
                >
                  {r}
                </span>
                {res.isNew && <p className="text-[10px] mt-1" style={{ color: "var(--success)" }}>New!</p>}
              </motion.div>
            );
          })}
        </div>
