...
export function IslandZones({ monsters, onEmptyClick }: { monsters: UserMonster[], onEmptyClick?: (zoneIndex: number) => void }) {
  return (
      ...
      {ZONE_NAMES.map((zone, i) => {
        ...
        return (
          <div key={zone} className="island-zone min-w-[80px]">
            <span className="font-lore text-xs mb-2" style={{ color: "var(--ink-secondary)" }}>{zone}</span>
            {monster ? (
              <MonsterCard monster={monster as any} compact={true} onRemove={() => onEmptyClick?.(slot)} />
            ) : (
...
function EmptyZoneSlot({ zoneIndex, onClick }: { zoneIndex: number; onClick?: () => void }) {
  return (
    ...
    >
      <span className="text-2xl" style={{ color: "var(--ink-secondary)" }}>+</span>
    </button>
  );
}
