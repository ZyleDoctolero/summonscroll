  ...
  return (
    <div className="relative w-full max-w-lg mx-auto py-12 flex flex-col items-center">
      ...
      <motion.div
        ...
        className="relative z-10 ss-card p-4 rounded-full border-2 text-center mb-12 shadow-[0_0_20px_rgba(0,0,0,0.5)]"
        style={{ borderColor: color, backgroundColor: "var(--bg-card)" }}
      >
        <p className="text-[10px] font-mono tracking-widest text-slate-400">ORIGIN</p>
      ...
      {milestones.map((m, i) => (
        ...
        >
          ...
          {m.active && (
            <div
              className="absolute top-[-3rem] bottom-[100%] left-1/2 w-0.5 bg-cyan-500 -translate-x-1/2 -z-10 shadow-[0_0_10px_rgba(0,240,255,0.8)]"
              style={{ height: "3rem" }}
            />
          )}
          ...
          <div
            ...
            style={{ borderLeftColor: m.active ? "var(--cyan)" : "var(--ink-tertiary)" }}
          >
            <div className="flex items-center gap-3 mb-2">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${m.active ? "bg-cyan-900/40 text-cyan-400" : "bg-slate-800 text-slate-500"}`}
              >
                <m.icon size={16} />
              </div>
              <div>
                <p className="text-xs font-bold text-white">{m.label}</p>
                <p className="text-[10px] font-mono text-cyan-400">REQUIRES {m.star}★</p>
              </div>
            </div>
            {m.value && (
      ...
      {currentStar >= 3 && currentStar < 10 && (
        <p className="text-[10px] text-center mt-4 text-cyan-500/60 font-mono">
