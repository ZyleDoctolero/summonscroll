...
function AltarPage() {
  ...
  if (profileQ.isLoading || bannersQ.isLoading) return <div className="min-h-screen grid place-items-center text-muted-foreground">Loading the Altar…</div>;
    ...
    if (!allRevealed) {
      ...
      return (
        <AppShell profile={profile}>
          <div className="fixed inset-0 z-50 flex flex-col items-center justify-center cursor-pointer ss-modal-backdrop" onClick={() => setRevealIndex((i) => Math.min(i + 1, pullResults.length - 1))}>
            <div className="ss-modal text-center max-w-xs" style={{ border: `2px solid ${RARITY_COLOR[r]}`, boxShadow: RARITY_GLOW[r] }}>
              ...
              <h2 className="text-2xl font-bold mb-2 text-gold-bright" style={{ color: RARITY_COLOR[r] }}>{current.monster.name}</h2>
              <span className="ss-chip" style={{ background: `${RARITY_COLOR[r]}20`, color: RARITY_COLOR[r], border: `1px solid ${RARITY_COLOR[r]}60` }}>{r}</span>
              <p className="text-xs mt-2 text-muted-foreground">{current.monster.element} · {current.monster.role}</p>
              {current.isNew && <p className="text-sm mt-2 font-medium text-success flex items-center gap-1 justify-center"><Icon name="sparkle" size={14} color="var(--success)" /> New!</p>}
              {current.transcendenceStone && <p className="text-sm mt-1 flex items-center gap-1 justify-center" style={{ color: "var(--gold-bright)" }}><Icon name="summon" size={14} /> Transcendence Stone</p>}
            </div>
            {pullResults.length > 1 && <div className="flex gap-1.5 mt-6">{pullResults.map((_, i) => <div key={i} className="w-2 h-2 rounded-full" style={{ background: i <= revealIndex ? "var(--gold-bright)" : "rgba(255,255,255,0.1)" }} />)}</div>}
      ...
      <AppShell profile={profile}>
        <div className="fixed inset-0 z-50 flex flex-col ss-modal-backdrop">
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 max-w-2xl mx-auto">
              {sorted.map((res, i) => { const r = res.monster.rarity; return (
                <div key={i} className="ss-card text-center" style={{ border: `1px solid ${RARITY_COLOR[r]}`, boxShadow: r !== "common" ? RARITY_GLOW[r] : undefined }}>
                  ...
                  <span className="ss-chip mt-1" style={{ background: `${RARITY_COLOR[r]}20`, color: RARITY_COLOR[r] }}>{r}</span>
          ...
          <div className="p-4 border-t border-border">
            <button onClick={() => setPullResults(null)} className="ss-btn ss-btn-primary w-full">Continue</button>
          </div>
        </div>
      </AppShell>
    );
  ...
  return (
    <AppShell profile={profile}>
      <div className="bg-atmos bg-atmos-altar p-6 md:p-10 max-w-4xl mx-auto min-h-screen">
        ...
        <p className="text-sm mb-8" style={{ color: "var(--ink-secondary)" }}>Spend Crystals to summon monsters.</p>
        ...
        {selectedBanner && (() => {
          ...
          return (
            <div className="ss-card-hero overflow-hidden">
              <div className="relative h-48 flex items-end p-6" style={{ background: "linear-gradient(180deg, rgba(255,213,79,0.15) 0%, var(--bg-pane) 100%)" }}>
                ...
                <div>
                  <h2 className="text-2xl font-bold mb-1" style={{ color: "var(--gold-bright)" }}>{selectedBanner.name}</h2>
                  <p className="text-sm" style={{ color: "var(--ink-secondary)" }}>{selectedBanner.realms?.icon} {selectedBanner.realms?.name ?? "All Realms"}</p>
                </div>
              </div>
              <div className="p-6 space-y-4">
                <div className="text-sm" style={{ color: "var(--ink-secondary)" }}>Balance: <span className="font-bold text-base flex items-center gap-1 inline-flex" style={{ color: balance >= cost1 ? "var(--gold-bright)" : "var(--danger)" }}><Icon name={icon as any} size={14} color={iconColor} /> {balance.toLocaleString()}</span></div>
                <div className="flex gap-3">
                  <button onClick={() => pullMut.mutate({ bannerId: selectedBanner.id, count: 1 })} disabled={balance < cost1 || pullMut.isPending}
                    className="ss-btn ss-btn-secondary flex-1">Pull ×1 — {cost1} <Icon name={icon as any} size={12} color={iconColor} /></button>
                  <button onClick={() => pullMut.mutate({ bannerId: selectedBanner.id, count: 10 })} disabled={balance < cost10 || pullMut.isPending}
                    className="ss-btn ss-btn-d-primary flex-[2] disabled:opacity-40" style={{ boxShadow: balance >= cost10 ? "0 0 24px rgba(255,213,79,0.3)" : "none" }}>
                    {pullMut.isPending ? "Summoning…" : <span className="flex items-center gap-1">★ Pull ×10 — {cost10} <Icon name={icon as any} size={12} color={iconColor} /> ★</span>}</button>
                </div>
              </div>
            </div>
          );
