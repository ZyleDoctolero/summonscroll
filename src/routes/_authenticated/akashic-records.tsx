...
function AkashicRecordsPage() {
  ...
  const { data: monsters } = useQuery({
    ...
    queryFn: async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return [];
      const { data, error } = await supabase.from('user_monsters').select('*, monster:monster_id(*)').eq('user_id', user.id);
      if (error) throw error;
      return data;
    }
  });
  ...
  const availableFodder = monsters.filter(m => m.id !== selectedTarget && m.current_star === requiredFodderCount);
