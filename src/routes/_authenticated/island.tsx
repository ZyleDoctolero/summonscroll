    ...
    <AppShell profile={profileQ.data.profile}>
      <div className="bg-atmos bg-atmos-hub relative min-h-screen">
        <div className="p-6 md:p-10 max-w-5xl mx-auto">
          ...
          {/* Weather banner */}
          <div className="ss-card mb-6" style={{ background: weatherBg, borderColor: "var(--ss-border)" }}>
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-mono" style={{ color: "var(--gold-bright)" }}>Power: {teamPower.toLocaleString()}</span>
            </div>
            <div className="grid grid-cols-5 gap-3">
              {[1, 2, 3, 4, 5].map((slot) => {
                ...
                if (um) {
                  ...
                  return (
                    <div key={slot} className="ss-card text-center relative" style={{
                      ...
                      <p className="text-[9px]" style={{ color: "var(--ink-secondary)" }}>Lvl {um.level} · Bond {Math.round(um.bond_percent)}%</p>
                      {fatigued && <div className="absolute top-1 right-1 text-[10px] px-1 rounded font-bold" style={{ background: "var(--danger)", color: "#fff" }}>⚡FATIGUE</div>}
                      <button onClick={() => slotMut.mutate({ userMonsterId: um.id, slot: null })}
                        className="mt-1 text-[9px] px-2 py-0.5 rounded font-semibold" style={{ color: "var(--danger)", background: "rgba(255,94,94,0.1)" }}>Remove</button>
                    </div>
                  );
                }
                return (
          ...
          {roster.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
              {roster.map((um: { id: string; monster: { name: string; rarity: string; element: string }; level: number; bond_percent: number; ascension_level: number; is_on_team: boolean }) => {
                ...
                return (
                  <div key={um.id} className="ss-card text-center p-2" style={{
      ...
      {/* Assign modal */}
      {assignSlot !== null && (
        <div className="fixed bottom-0 left-0 right-0 z-40 p-4 border-t ss-modal rounded-b-none" style={{ background: "var(--bg-stage)" }}>
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <p className="text-sm font-semibold" style={{ color: "var(--gold-bright)" }}>Select a monster for Slot {assignSlot}</p>
            <button onClick={() => setAssignSlot(null)} className="ss-btn ss-btn-secondary py-1 text-xs">Cancel</button>
          </div>
        </div>
      )}
    </AppShell>
