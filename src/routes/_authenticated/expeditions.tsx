...
function ExpeditionsPage() {
  ...
  if (profileQ.isLoading) {
    return <div className="min-h-screen grid place-items-center" style={{ color: "var(--ink-secondary)" }}>Loading expeditions…</div>;
  }
      ...
      <div className="bg-atmos bg-atmos-expedition p-6 md:p-10 max-w-3xl mx-auto min-h-screen">
        ...
        <div className="ss-card mb-6">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs uppercase tracking-wider" style={{ color: "var(--ink-secondary)" }}>Stamina</span>
        ...
        >
          <div className="flex justify-between items-start mb-3">
            <div>
              <p className="text-[10px] uppercase tracking-widest mb-1" style={{ color: "var(--ink-secondary)" }}>
          ...
          <p className="text-sm italic mb-4" style={{ color: "var(--ink-secondary)" }}>{def.flavor}</p>

          <div className="grid grid-cols-2 gap-2 text-xs mb-4" style={{ color: "var(--ink-secondary)" }}>
            <div className="ss-pane">
              <div className="text-[10px] uppercase" style={{ color: "var(--ink-tertiary)" }}>Primary drop</div>
              <div className="font-bold mt-0.5 flex items-center gap-1" style={{ color: "var(--ink-primary)" }}>
                <Icon name="stone" size={12} color="var(--ink-secondary)" /> {def.primaryStone}
              </div>
            </div>
            <div className="ss-pane">
              <div className="text-[10px] uppercase" style={{ color: "var(--ink-tertiary)" }}>Elite chance</div>
              <div className="font-bold mt-0.5" style={{ color: "var(--ink-primary)" }}>~5% per run</div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {runMut.isPending ? "Running…" : <span className="flex items-center gap-1">Run ×1 — 5<Icon name="stamina" size={12} /></span>}
            </button>
            <button
            ...
            >
              {runMut.isPending ? "Running…" : <span className="flex items-center gap-1">★ Run ×5 — 25<Icon name="stamina" size={12} /> ★</span>}
            </button>
          </div>
        </div>
          ...
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((label, i) => {
            ...
            return (
              <div
                ...
                style={isToday ? { background: "rgba(255,213,79,0.1)", borderColor: "var(--gold-bright)" } : undefined}
              >
                <div className="text-[10px] uppercase tracking-wider" style={{ color: isToday ? "var(--gold-bright)" : "var(--ink-tertiary)" }}>{label}</div>
                <div className="text-[9px] mt-1" style={{ color: isToday ? "var(--ink-primary)" : "var(--ink-tertiary)" }}>{EXPEDITIONS[t].name.split(" ")[0]}</div>
              </div>
            );
          })}
        </div>
        ...
        {results && (
          ...
          >
            <div
              ...
              className="ss-modal"
            >
              <h2 className="text-xl font-bold mb-1 text-center" style={{ color: "var(--gold-bright)" }}>
              ...
              <div className="ss-pane mb-4">
                <p className="text-[10px] uppercase tracking-wider mb-2" style={{ color: "var(--ink-secondary)" }}>Loot</p>
                {results.drops.length === 0 ? (
                  <p className="text-xs" style={{ color: "var(--ink-tertiary)" }}>The realm yielded nothing this time.</p>
                ) : (
                  <ul className="space-y-1">
                    {results.drops.map((d, i) => (
                      <li key={i} className="flex justify-between text-sm">
                        <span className="flex items-center gap-1.5">
                          <Icon name={(DROP_ICONS[d.type] ?? "sparkle") as any} size={14} color="var(--gold-glow)" />
                          <span style={{ color: "var(--ink-primary)" }}>{d.name}</span>
                        </span>
                        <span className="font-mono font-bold" style={{ color: "var(--gold-bright)" }}>×{d.qty}</span>
                      </li>
                    ))}
                  </ul>
                )}
