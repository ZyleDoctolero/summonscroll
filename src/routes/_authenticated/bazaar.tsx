  ...
  return (
    <AppShell profile={profile}>
      <div className="p-6 md:p-10 max-w-4xl mx-auto">
        <h1 className="t-h1 text-3xl font-bold mb-1" style={{ color: "var(--gold-bright)" }}>The Bazaar</h1>
        <p className="text-sm mb-6" style={{ color: "var(--ink-secondary)" }}>Spend your hard-earned crystals on equipment, potions, and quest scrolls.</p>

        {/* Balance */}
        <div className="flex gap-4 mb-6 text-sm" style={{ color: "var(--ink-secondary)" }}>
          <span><Icon name="gold" size={13} color="var(--gold-bright)" /> <b className="t-mono" style={{ color: "var(--gold-bright)" }}>{profile.gold.toLocaleString()}</b></span>
          <span><Icon name="crystal" size={13} color="var(--cyan)" /> <b className="t-mono" style={{ color: "var(--cyan)" }}>{profile.crystals.toLocaleString()}</b></span>
          <span><Icon name="seal" size={13} color="var(--violet)" /> <b className="t-mono" style={{ color: "var(--violet)" }}>{profile.pact_seals}</b></span>
        </div>
        ...
        {/* Armoire special UI */}
        {tab === "armoire" && (
          <div className="ss-card-hero text-center mb-6 p-8">
            <div className="text-5xl mb-4">🗄</div>
            <h2 className="text-xl font-bold mb-2" style={{ color: "var(--gold-bright)" }}>Enchanted Armoire</h2>
            <p className="text-sm mb-4" style={{ color: "var(--ink-secondary)" }}>Spend 100💎 for a chance at rare equipment, food for your pets, or bonus XP!</p>
            <button
              onClick={() => {
              ...
              className="ss-btn ss-btn-d-primary disabled:opacity-40">
            ...
            {items.map((item: { id: string; name: string; description: string; price: number; currency: string; effect_type: string; effect_value: number }) => {
              ...
              return (
                <div key={item.id} className="ss-card">
                  <h3 className="font-bold text-sm mb-1" style={{ color: "var(--ink-primary)" }}>{item.name}</h3>
                  <p className="text-xs mb-3" style={{ color: "var(--ink-secondary)" }}>{item.description}</p>
                  <div className="flex items-center justify-between">
                    <span className="t-mono font-bold text-sm" style={{ color: canAfford ? "var(--gold-bright)" : "var(--danger)" }}>
                      {icon} {item.price}
                    </span>
                    <button
