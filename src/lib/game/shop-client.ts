...
export async function purchaseItem(shopItemId: string, quantity = 1) {
  ...
  const field = item.currency === "pact_seals" ? "pact_seals" : item.currency === "gold" ? "gold" : "crystals";
  const balance = profile[field] as number;
  if (balance < totalCost) throw new Error(`Insufficient funds. Need ${totalCost}, have ${balance}.`);

  await supabase.from("profiles").update({ [field]: balance - totalCost }).eq("id", user.id);
  ...
  if (item.effect_type === "heal_hp") {
    ...
    if (roll < 0.4) {
      const { data: gear } = await supabase.from("equipment").select("*").eq("is_armoire_exclusive", true);
      if (gear?.length) {
      ...
      await supabase.from("profiles").update({ xp: profile.xp + xpGain }).eq("id", user.id);
      return { ok: true, reward: { type: "xp", name: `${xpGain} XP` } };
    }
  } else if (item.equipment_id) {
    for (let i = 0; i < quantity; i++) {
      await supabase.from("user_equipment").insert({ user_id: user.id, equipment_id: item.equipment_id });
    }
  } else {
    await upsertInventory(user.id, item.effect_type ?? "potion", item.name, quantity);
  }

  await supabase.from("purchases").insert({ user_id: user.id, shop_item_id: shopItemId, quantity, total_cost: totalCost });
...
export async function equipItem(userEquipmentId: string) {
  ...
  await supabase.from("user_equipment").update({ is_equipped: false }).eq("user_id", user.id).eq("is_equipped", true);

  // Equip new
  await supabase.from("user_equipment").update({ is_equipped: true }).eq("id", userEquipmentId);
  await supabase.from("profiles").update({ [profileField]: ue.equipment_id }).eq("id", user.id);
  ...
  for (const e of equipped ?? []) {
    const eqRow = e.equipment as { str_bonus: number; int_bonus: number; con_bonus: number; per_bonus: number; class_affinity: string | null };
  ...
  await supabase.from("profiles").update({ str_stat: str, int_stat: int_, con_stat: con, per_stat: per }).eq("id", user.id);
  return { ok: true };
}

async function upsertInventory(userId: string, itemType: string, itemName: string, qty = 1) {
  const { data: existing } = await supabase.from("inventory").select("id, quantity").eq("user_id", userId).eq("item_type", itemType).eq("item_name", itemName).maybeSingle();
  if (existing) await supabase.from("inventory").update({ quantity: existing.quantity + qty }).eq("id", existing.id);
  else await supabase.from("inventory").insert({ user_id: userId, item_type: itemType, item_name: itemName, quantity: qty });
}
