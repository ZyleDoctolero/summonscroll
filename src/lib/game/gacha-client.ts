...
export async function pullBanner(bannerId: string, count: 1 | 10) {
  ...
  if (balance < totalCost)
    throw new Error(
      `Insufficient ${isPactSeal ? "Pact Seals" : "Crystals"}. Need ${totalCost}, have ${balance}.`,
    );

  // Load monsters for banner
  const monstersQuery = supabase
    .from("monsters")
    .select("*")
  ...
  const currencyUpdate = isPactSeal
    ...
    : { crystals: profile.crystals - totalCost };
  await supabase.from("profiles").update(currencyUpdate).eq("id", user.id);

  const newInserts = results
    .filter((r) => r.isNew)
    .map((r) => ({ user_id: user.id, monster_id: r.monster.id }));
  ...
  for (const dupe of results.filter((r) => !r.isNew && !r.transcendenceStone)) {
    const { data: existing } = await supabase
      .from("user_monsters")
      .select("id, bond_percent, star_level")
      .eq("monster_id", dupe.monster.id)
      .maybeSingle();
    if (existing)
      await supabase
        .from("user_monsters")
        .update({ bond_percent: Math.min(100, existing.bond_percent + 10) })
        .eq("id", existing.id);
  }
  ...
  await supabase.from("pulls").insert(
    results.map((r) => ({
      ...
      currency_spent: isPactSeal ? "pact_seals" : "gems",
      amount_spent: Math.round(totalCost / count),
    })),
  );
