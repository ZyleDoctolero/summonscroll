...
-- Clean it if rerunning
DELETE FROM public.loot_tables WHERE item_type = 'material' AND item_name IN ('Flame Core', 'Mind Core', 'Aqua Core');

INSERT INTO public.loot_tables (item_type, item_name, drop_chance, category_requirement) VALUES
('material', 'Flame Core', 0.15, 'fitness'),
('material', 'Flame Core', 0.15, 'sport'),
('material', 'Mind Core', 0.15, 'study'),
('material', 'Mind Core', 0.15, 'work'),
('material', 'Aqua Core', 0.15, 'chores'),
('material', 'Aqua Core', 0.15, 'health');

-- 2. Guild Boss MVP
CREATE TABLE IF NOT EXISTS public.guild_bosses (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  guild_id uuid NOT NULL REFERENCES public.guilds(id) ON DELETE CASCADE,
  name text NOT NULL DEFAULT 'Demonic Tribulation Beast',
...
GRANT SELECT ON public.loot_tables TO authenticated, anon;
GRANT SELECT ON public.guild_bosses TO authenticated, anon;
GRANT UPDATE ON public.guild_bosses TO authenticated;
...
DECLARE
  ...
  v_guild_id  uuid;
  v_boss      record;
  v_cp        int := 50; -- Default placeholder for CP
  v_loot      record;
BEGIN
  ...
  v_mult := CASE t.difficulty WHEN 'trivial' THEN 0.1 WHEN 'easy' THEN 1 WHEN 'medium' THEN 1.5 WHEN 'hard' THEN 2 ELSE 1 END;
  ...
  IF v_is_pos THEN
    ...
    IF v_last_task IS NOT NULL AND (now() - v_last_task) < interval '1 hour' THEN v_combo := v_combo + 1; ELSE v_combo := 1; END IF;
  ...
  UPDATE public.tasks SET value = v_val, streak = v_streak, completed = v_completed, last_completed_date = v_lcd, last_completed_at = CASE WHEN v_is_pos THEN now() ELSE last_completed_at END WHERE id = p_task_id;
  ...
  UPDATE public.profiles SET gold = v_new_gold, crystals = v_new_crystals, hp = v_new_hp, xp = v_new_xp, level = v_new_level, deaths = v_deaths, combo_count = v_combo, last_task_time = CASE WHEN v_is_pos THEN now() ELSE last_task_time END WHERE id = v_uid;

  -- Habitica Engine: Time Skips
  IF v_is_pos THEN
  ...
  -- Habitica Engine: Guild Boss Damage
  SELECT guild_id INTO v_guild_id FROM public.guild_members WHERE user_id = v_uid LIMIT 1;
  IF v_guild_id IS NOT NULL THEN
    SELECT * INTO v_boss FROM public.guild_bosses WHERE guild_id = v_guild_id LIMIT 1;
    IF FOUND THEN
      IF v_is_pos THEN
        UPDATE public.guild_bosses SET hp = greatest(0, hp - v_cp) WHERE id = v_boss.id;
      ELSE
        UPDATE public.profiles SET hp = greatest(0, hp - 50) WHERE id = v_uid;
      END IF;
    END IF;
  END IF;

  -- Habitica Engine: Category-Specific Cores via Loot Tables
  IF v_is_pos THEN
    v_per_bonus := 1 + COALESCE(p.per_stat,0) * 0.005;
    v_drop_roll := random() / (v_per_bonus * v_collector);

    SELECT * INTO v_loot FROM public.loot_tables WHERE category_requirement = t.category AND v_drop_roll < drop_chance ORDER BY random() LIMIT 1;
    
    IF v_loot IS NOT NULL THEN
      v_drop := jsonb_build_object('type', v_loot.item_type, 'name', v_loot.item_name);
    ELSE
      -- Fallback
      IF v_drop_roll < 0.03 THEN v_drop := jsonb_build_object('type','egg','name',(ARRAY['Wolf','Dragon','Phoenix','Serpent','Griffin','Owl','Bear','Fox'])[floor(random()*8+1)] || ' Egg');
      ELSIF v_drop_roll < 0.06 THEN v_drop := jsonb_build_object('type','realm_potion','name',(ARRAY['Arcane','Chaos','Void','Death','Nature','Divine','Dread','Digital'])[floor(random()*8+1)] || ' Potion');
      ELSIF v_drop_roll < 0.10 THEN v_drop := jsonb_build_object('type','food','name',(ARRAY['Meat','Fish','Fruit','Cheese','Honey'])[floor(random()*5+1)]);
      ELSIF v_drop_roll < 0.15 THEN v_drop := jsonb_build_object('type','material','name',(ARRAY['Iron Ore','Shadow Essence','Void Core','Light Crystal'])[floor(random()*4+1)]);
      END IF;
    END IF;
    ...
    IF array_length(v_stat_tags,1) > 0 THEN
      FOR m IN SELECT um.id, mo.name AS monster_name, CASE mo.role WHEN 'attacker' THEN 'str' WHEN 'tank' THEN 'con' WHEN 'healer' THEN 'con' WHEN 'support' THEN 'int' WHEN 'debuffer' THEN 'per' ELSE 'str' END AS stat, um.bond_percent, um.growth_xp FROM public.user_monsters um JOIN public.monsters mo ON mo.id = um.monster_id WHERE um.user_id = v_uid
      LOOP
        IF m.stat = ANY(v_stat_tags) THEN
          UPDATE public.user_monsters SET bond_percent = least(100, COALESCE(m.bond_percent,0) + 0.5), growth_xp = COALESCE(m.growth_xp,0) + 1 WHERE id = m.id;
