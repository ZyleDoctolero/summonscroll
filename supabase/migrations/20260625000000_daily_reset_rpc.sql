-- Migration: Server-Side Daily Reset (LOGIC-02)
...
BEGIN
  ...
  IF v_profile.last_cron_date IS NOT NULL AND v_profile.last_cron_date >= p_today THEN
  ...
  IF v_last_cron IS NOT NULL THEN
    v_diff := p_today::date - v_last_cron;
    ...
    FOR i IN 1 .. array_length(v_days, 1) LOOP
      ...
      FOR v_task IN 
        ...
        WHERE user_id = p_user_id AND type = 'daily' AND archived = false 
          AND (last_completed_date IS NULL OR last_completed_date < v_day::text)
      LOOP
        IF v_task.schedule_days @> to_jsonb(v_dow) THEN
      ...
      IF v_day_missed_count > 0 THEN
        IF v_freezes > 0 THEN
          ...
          FOR v_task IN SELECT * FROM public.tasks WHERE user_id = p_user_id AND type = 'daily' AND archived = false AND (last_completed_date IS NULL OR last_completed_date < v_day::text) LOOP
             IF v_task.schedule_days @> to_jsonb(v_dow) THEN
               v_diff_mult := CASE v_task.difficulty WHEN 'trivial' THEN 0.1 WHEN 'easy' THEN 1.0 WHEN 'medium' THEN 1.5 WHEN 'hard' THEN 2.0 ELSE 1.0 END;
  ...
  WHERE user_id = p_user_id AND type = 'daily' AND archived = false AND schedule_days @> to_jsonb(v_dow);
